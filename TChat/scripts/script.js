            ...
            if (state.mode === 'TUI_POST_WRITE') {
                ...
                reader.onload = async (event) => {
                    try {
                        ...
                        // Refocus the body textarea after image upload
                        setTimeout(() => {
                        ...
                        // Still refocus textarea even on error
                        setTimeout(() => {
                            const bodyTextarea = document.getElementById('post-body');
                            if (bodyTextarea) {
                                bodyTextarea.focus();
 ...
 |__   __| / ____| |         | |  
    ...
    |_|    \\_____|_| |_|\\__,_|\\__|  v2.0
        ...
        function closeAllWindows() {
            ...
            // Re-enable publish button if it exists
        ...
        document.addEventListener('keydown', (event) => {
            ...
            // Prevent input in main command line when in TUI_POST_WRITE mode
            if (state.mode === 'TUI_POST_WRITE') {
                // Allow typing in post writer fields, but prevent in main input
                ...
                // If Escape, let it through to close window (handled above)
                if (event.key === 'Escape') {
                    return; // Let the Escape handler above handle it
                }
                
                // If typing and not in a post field, focus the body textarea
        ...
        function openPostWriter() {
            ...
            // Setup publish and cancel buttons
            const publishBtn = document.getElementById('post-publish-btn');
        ...
        async function publishPost() {
            ...
            // Disable publish button and set flag
            isPublishing = true;
            const publishBtn = document.getElementById('post-publish-btn');
            if (publishBtn) {
